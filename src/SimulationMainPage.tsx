...
const SimulationMainPage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (currentScenarioIndex > 0 && showPriorityMessage) {
  ...
  }, [currentScenarioIndex, showPriorityMessage]);
  ...
  const handleConfirmDecision = () => {
    ...
    if (currentScenarioIndex < scenarios.length - 1) {
      ...
      setTimeout(() => {
        ...
        setShowPriorityMessage(false); // Reset priority message visibility for next scenario
      }, 3000);
    }
    ...
    <div className="h-screen bg-gray-50 p-4 flex flex-col">
      <div className="max-w-6xl mx-auto w-full flex-1 flex flex-col">
        ...
        <div className="bg-white rounded-lg shadow-md p-4 flex-1 flex flex-col overflow-hidden">
          ...
          {!showPriorityMessage && currentScenarioIndex > 0 && (
            <div className="mb-4">
              <button
                onClick={() => setShowPriorityMessage(true)}
                className="w-full flex items-center justify-center gap-2 py-2 px-4 rounded-lg text-white font-medium transition-colors duration-200 bg-green-600 hover:bg-green-700"
              >
                Continue
              </button>
            </div>
          ...
          {priorityMessage && showPriorityMessage && currentScenarioIndex > 0 && (
